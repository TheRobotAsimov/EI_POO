...
public class GraficarNumNeumaticos {
    
    public void graficar (JPanel panelGrafica){
        
        JFreeChart grafico_valores = ChartFactory.createBarChart3D(
            "Numero de Neumaticos",    // Nombre del grafico
            "Equipo", // Nombre de las barras (eje x)
    ...
    public DefaultCategoryDataset datos(){
        ...
        try {
            ...
            ResultSet rs = con.comando.executeQuery("select marca, cantNeuma as cantidad from equipo;");
            while(rs.next()){
                datos.addValue(rs.getInt("cantidad"),rs.getString("marca"),rs.getString("marca"));
            }
